...
def debug_encoder_structure(encoder, model_name, device='cuda'):
    """Debug function để kiểm tra cấu trúc encoder output"""
    print(f"Debugging {model_name} encoder structure...")
...
def get_encoder_embedding(encoder_output, model_name):
    """Extract embedding from encoder output based on model type"""
    if model_name == 'blip2':
        if hasattr(encoder_output, 'last_hidden_state'):
            return encoder_output.last_hidden_state.mean(dim=1)
        elif isinstance(encoder_output, tuple):
            return encoder_output[0].mean(dim=1)
        else:
        ...
        if isinstance(encoder_output, tuple):
            return encoder_output[1]  # BLIP thường dùng index 1
        else:
...
def uap_sgd(model, model_name, encoder, tokenizer, image_processor, image_mean, image_std, clip_model, loader, nb_epoch, eps, c = 0.1, targeted=False, lr=0.01, nb_imgs=1000):
    ...
    # Debug encoder structure
    debug_encoder_structure(encoder, model_name)
    ...
    for i, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            x_emb = get_encoder_embedding(encoder_output, model_name).detach()
        ...
        for epoch in range(nb_epoch):
            ...
            x_adv_encoder_output = encoder(x_adv)
            x_adv_emb = get_encoder_embedding(x_adv_encoder_output, model_name)
...
if __name__ == '__main__':    
    ...
    for loss in total_losses:
        if loss:  # Check if loss list is not empty
            last_loss = loss[-1]
            mean_loss += last_loss
        
    # Calculate mean CLIP losses
    ...
    if len(clip_losses) > 0:
        ...
        print("No valid image pairs found for attack.")